...
public class CustomSpinner extends android.support.v7.widget.AppCompatTextView {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        try {
            backgroundColor = ta.getColor(R.styleable.CustomSpinner_cs_background_color, Color.WHITE);
            textColor = ta.getColor(R.styleable.CustomSpinner_cs_text_color, defaultColor);
            arrowColor = ta.getColor(R.styleable.CustomSpinner_cs_arrow_tint, textColor);
            hideArrow = ta.getBoolean(R.styleable.CustomSpinner_cs_hide_arrow, false);
            popupWindowMaxHeight = ta.getDimensionPixelSize(R.styleable.CustomSpinner_cs_dropdown_max_height, 0);
            popupWindowHeight = ta.getLayoutDimension(R.styleable.CustomSpinner_cs_dropdown_height,
        ...
        left = right = bottom = top = resources.getDimensionPixelSize(R.dimen.cs__padding_top);
        if (rtl) {
            right = resources.getDimensionPixelSize(R.dimen.cs__padding_left);
        } else {
            left = resources.getDimensionPixelSize(R.dimen.cs__padding_left);
        }
        ...
        if (!hideArrow) {
            arrowDrawable = Utils.getDrawable(context, R.drawable.cs__arrow).mutate();
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            popupWindow.setBackgroundDrawable(Utils.getDrawable(context, R.drawable.cs__drawable));
        } else {
            popupWindow.setBackgroundDrawable(Utils.getDrawable(context, R.drawable.cs__drop_down_shadow));
        }
    ...
    public <T> void setItems(@NonNull List<T> items) {
        ...
        adapter = new CustomSpinnerAdapter<>(getContext(), items).setTextColor(textColor);
    ...
    public void setAdapter(@NonNull ListAdapter adapter) {
        this.adapter = new CustomSpinnerAdapterWrapper(getContext(), adapter);
        setAdapterInternal(this.adapter);
    }

    public <T> void setAdapter(CustomSpinnerAdapter<T> adapter) {
    ...
    private int calculatePopupWindowHeight() {
        ...
        float listViewHeight = adapter.getCount() * getResources().getDimension(R.dimen.cs__item_height);
